  ...
  useEffect(() => {
    if (isOpen) {
      if (editingSchedule) {
        ...
        setIsBankScheduled(!!editingSchedule.isBankScheduled);
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center justify-between p-2 mt-4 bg-indigo-50 dark:bg-indigo-900/10 rounded-lg border border-indigo-100 dark:border-indigo-800/30">
              <label className="text-xs font-bold text-indigo-700 dark:text-indigo-300 flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={isBankScheduled}
                  onChange={(e) => setIsBankScheduled(e.target.checked)}
