...
const Reports: React.FC<ReportsProps> = ({ data }) => {
  const [activeTab, setActiveTab] = useState<'financial' | 'categories' | 'accounts' | 'detailed' | 'tithing' | 'audit' | 'accounting' | 'council'>('financial');
  ...
  const getIncomeComposition = () => {
    ...
    financialData.transactions
      ...
      .forEach(t => {
        ...
        if (catName.includes('dízimo') || catName.includes('dizimo')) {
          dizimos += t.amount;
    ...
    return [
      { name: 'Dízimos', value: dizimos, color: '#8b5cf6' },
      { name: 'Ofertas', value: ofertas, color: '#3b82f6' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex overflow-x-auto border-b border-gray-200 dark:border-slate-700 pb-1 scrollbar-hide">
        {[{ id: 'financial', label: 'Relatório Financeiro', icon: Wallet }, { id: 'categories', label: 'Por Categoria', icon: PieIcon }, { id: 'council', label: 'Conselho Fiscal', icon: Scale }, { id: 'tithing', label: 'Dízimos & Ofertas', icon: FileCheck }, { id: 'accounting', label: 'Contabilidade', icon: Calculator }, { id: 'audit', label: 'Auditoria', icon: History }].map(tab => (
